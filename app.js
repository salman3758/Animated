...
dbRef.on('value', snap => voltage.innerText = snap.val());


var current  = document.getElementById('current');
var dbRef2 = firebase.database().ref().child('data2');
dbRef2.on('value', snap => current.innerText = snap.val());
...
// }
  ...
  document.getElementById("pf").innerHTML="0.00";
}
...
document.getElementById("rp").innerHTML= ap;

// Apparent Power = V*I 
document.getElementById("app").innerHTML= ap1.toFixed(2);
...
// PLOTLY.JS
   ...
   dbRef2.on('value', (snapshot) => {
    const data44 = snapshot.val();
  arrays.push(data44);

});

dbRef.on('value', (snapshot) => {
  const data22 = snapshot.val();
arrays2.push(data22);
